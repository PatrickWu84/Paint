...
let make_controller (v: 'a) : 'a value_controller =
  ...
  let value = ref v in
  { add_change_listener = (fun listener -> listeners := listener :: !listeners);
    get_value = (fun () -> !value);
    change_value = (fun x ->
      value := x;
      List.iter (fun listener -> listener x) !listeners)
  }
  (*let value = {contents = v} in
  let list = ref [(fun _ -> ())] in
  {add_change_listener = (fun x -> list := x :: !list);
  get_value = (fun () -> !value);
  change_value = (fun x -> value := x); 
  List.iter (fun list -> list x) !list)}*)
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  let checked = make_controller init in
  let w = { repaint = (fun (g: Gctx.gctx) -> 
              if checked.get_value() then Gctx.fill_rect g (0,0) (10,10););
            handle = mouseclick_listener 
            (fun () -> checked.change_value (not (checked.get_value ())));
            size = (fun _ -> (10, 10))} in
  let (l, _) = label s in
  let l' = {l with handle = mouseclick_listener 
            (fun () -> checked.change_value (not (checked.get_value ())))} in
  let w' = border (hpair (border w) l') in
  (w', checked)


  (*let state = {contents = checked.get_value()} in
  let change =  (fun _ -> checked.change_value (not checked.get_value()))in*)
...
let slider (init_x_pos: int) : widget * int value_controller =
  (* keep track of slider mode. false means slider has not been clicked yet, 
  true means it has *)
  let mode = {contents = false} in
  (** initial value of slider *)
  let x_pos = make_controller init_x_pos in
  let w = {repaint = (fun (g: Gctx.gctx) ->
            Gctx.draw_rect g (0,0) (100, 10);
            Gctx.fill_rect g (0,0) (x_pos.get_value(), 10););
          handle = 
            (fun (g: Gctx.gctx) (e: Gctx.event) ->
            let (mouse_x, mouse_y) = Gctx.event_pos e g in (* mouse position *)
              if event_within g e (100,10) then (
              begin match (Gctx.event_type e) with
                |Gctx.MouseDown ->
                  if mode.contents then () else mode.contents <- true
                |Gctx.MouseDrag ->
                  if mode.contents
                  then if mouse_x < 1 then x_pos.change_value 1 
                        else if mouse_x > 100 then x_pos.change_value 100
                        else x_pos.change_value mouse_x
                  else mode.contents <- true
                |Gctx.MouseUp -> 
                  if mode.contents 
                  then x_pos.change_value mouse_x
                  else mode.contents <-true
                | _ -> ()
                end)
              else ());
          size = (fun _ -> (100, 10))} in
  let (l,_) = label (string_of_int init_x_pos) in
  let l' = {l with repaint = (fun (g: Gctx.gctx) ->
        Gctx.draw_string g (0,0) (string_of_int (x_pos.get_value ())))} in
  let w' = hpair (border w) l' in
  (w', x_pos)
