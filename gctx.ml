...
type gctx = {
  ...
  thickness: int; (** the current pen thickness *)
  }
...
(** also sets the line width of the pen*)
...
(** Draw an ellipse using the bounding box method. Takes in oppposite corners
of a rectangle and draws ellipse so that it fits exactly inside *)
let draw_bounding_ellipse (g: gctx) (p1: position) (p2: position) : unit = 
  set_graphics_state g;
  let (x1, y1) = ocaml_coords g p1 in
  let (x2, y2) = ocaml_coords g p2 in
  ...
  let px = (x1 + x2) / 2 in
  let py = (y1 + y2) / 2 in
  Graphics.draw_ellipse px py rx ry
...
(** Draw a one pixel point at the given position *)
let draw_point (g: gctx) (p: position) : unit = 
  set_graphics_state g;
  let (x, y) = ocaml_coords g p in
  Graphics.plot x y

(** Draw one pixel points at given positions *)
